...
object DatabaseModule {
    ...
    suspend fun prepopulateDatabase(context: Context): List<Patient> = withContext(Dispatchers.IO) {
            ...
            reader.forEachLine { line ->
                ...
                if (tokens.size >= 63) {
                    try {
                        ...
                        Log.e(TAG, "Error parsing line $lineNumber: $line", ex)
                    }
                } else {
                    Log.e(TAG, "Invalid line $lineNumber (not enough columns): $line")
                }
                lineNumber++
            }
            reader.close()
        } catch (e: Exception) {
            Log.e(TAG, "Error reading CSV file for prepopulation", e)
        }
